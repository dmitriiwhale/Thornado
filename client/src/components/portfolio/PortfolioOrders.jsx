import React from 'react'
import { fmt } from '../../lib/portfolioAdapters.js'

export default function PortfolioOrders({ rows, query, embedded = false }) {
  const wrapperClass = embedded ? '' : 'rounded-xl border border-white/10 bg-[rgba(12,14,32,0.72)] p-5 backdrop-blur-md'
  return (
    <section className={wrapperClass}>
      <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">
        Active orders
      </h3>
      {query.isLoading && <p className="mt-3 text-sm text-slate-400">Loading orders...</p>}
      {!query.isLoading && query.error && (
        <p className="mt-3 text-sm text-amber-200">Orders unavailable in current API response.</p>
      )}
      {!query.isLoading && !query.error && rows.length === 0 && (
        <p className="mt-3 text-sm text-slate-400">No active orders.</p>
      )}
      {rows.length > 0 && (
        <div className="mt-3 overflow-auto">
          <table className="w-full min-w-[620px] text-left text-xs text-slate-300">
            <thead className="text-slate-500">
              <tr>
                <th className="py-2">Market</th>
                <th className="py-2">Side</th>
                <th className="py-2">Price</th>
                <th className="py-2">Size</th>
                <th className="py-2">Status</th>
                <th className="py-2">Created</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="border-t border-white/5">
                  <td className="py-2 text-violet-100">{row.market}</td>
                  <td className="py-2">{row.side}</td>
                  <td className="py-2">{fmt.number(row.price)}</td>
                  <td className="py-2">{fmt.number(row.size)}</td>
                  <td className="py-2">{row.status}</td>
                  <td className="py-2">{fmt.datetime(row.createdAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </section>
  )
}
